...
def check_feature_set(feature_sets):
    ...
    for set in feature_sets.keys():
        if np.any(np.isnan(np.array(feature_sets[set]))):
...
def apply_nucleotide_features(seq_data_frame, order, num_proc, include_pos_independent, max_index_to_use, prefix=""):
    ...
    if include_pos_independent:
        ...
        assert not np.any(np.isnan(np.array(feat_pd))), "nans here can arise from sequences of different lengths"
        assert not np.any(np.isnan(np.array(feat_pi))), "nans here can arise from sequences of different lengths"
