...
import pandas
import numpy
import featurization_py3 as feat
...
def coordToFasta(regions, fasta_file, outputDir, targetSize, evalAndPrintFunc, nonOver, indexDir, genome, strand, ext):
    ...
    for region in regions:
        ...
        output = output[0]
        exons = output.decode("utf-8").split("\n")
...
def parse_primer3_output(target, region, primer3output, primerFastaFile):
    ...
    for line in primer3output.decode().split("\n"):
        if line[0] == "=":
            break
...
def dump_genbank_file(seq, target, restSites, primers, outputDir, geneID, lociStart, strand):
    ...
    record = SeqRecord(seq, description=desc, name="CHOPCHOP", id=name)
...
def pairPrimers(primerAttributes, primerList, outputDir):
    ...
    for primer in primerList:
        ...
        s = 0
        if side == "RIGHT": s = 1
        if not guide in primers: primers[guide] = {}
        if not primerPairID in primers[guide]: primers[guide][primerPairID] = [None, None]
...
def FastaToViscoords(sequences, strand):
    ...
    return list(zip(exonstart, exonend, exonsequence))
...
def print_genbank(mode, name, seq, exons, targets, chrom, seq_start, seq_end, strand, output_file, description): # different than other dump_gb
    ...
    if len(loci) > 10: # almost always...
        loci = name[-10:]
        record = SeqRecord(seq, description=description,name=name, id=loci)
...
def main():
    ...
    # -J -BED -G hg38 -filterGCmin 30 -filterGCmax 70 -t WHOLE -n N -R 4 
    ...
    global ISOFORMS
    ISOFORMS = args.isoforms # 默认参数中没有设置isoforms,取默认值False

    # Add TALEN length
    args.taleMin += 18 # 默认参数中没有设置taleMin,取默认值14,14+18=32
    args.taleMax += 18# 默认参数中没有设置taleMax,取默认值20,20+18=38
    ...
    #if MODE == CRISPR:return CRISPR_DEFAULT[index]
    #elif MODE == TALENS:return TALEN_DEFAULT[index]
    #elif MODE == CPF1:return CPF1_DEFAULT[index]
    #elif MODE == NICKASE:return NICKASE_DEFAULT[index]

    #sys.stderr.write("Unknown model %s\n" % f)
    #sys.exit(EXIT['PYTHON_ERROR'])
    
    # MODE取值Cas9(CRISPR),下面返回CRISPR_DEFAULT内的内容:
    # CRISPR_DEFAULT = {"GUIDE_SIZE" : 20,
    #                   "PAM": "NGG",
    #                   "MAX_OFFTARGETS" : 300,
    #                   "MAX_MISMATCHES" : 3,
    ...
    #                   "SCORE_FOLDING" : True}
    print(args.MODE)
    args.scoreGC = mode_select(args.scoreGC, "SCORE_GC", args.MODE) # 取值False
    args.scoreSelfComp = mode_select(args.noScoreSelfComp, "SCORE_FOLDING", args.MODE) # 取值True
    args.PAM = mode_select(args.PAM, "PAM", args.MODE) # 取值 NGG
    args.guideSize = mode_select(args.guideSize, "GUIDE_SIZE", args.MODE) + len(args.PAM) # 取值23
    args.maxMismatches = mode_select(args.maxMismatches, "MAX_MISMATCHES", args.MODE) # 取值3
    args.maxOffTargets = mode_select(args.maxOffTargets, "MAX_OFFTARGETS", args.MODE) # 取值300

    # Add TALEN length
    args.nickaseMin += args.guideSize # 取值10+23=33
    args.nickaseMax += args.guideSize # 取值31+23=54

    if args.scoreSelfComp: # 取值True
        if args.backbone: # backbone取值AGGCTAGTCCGT
            tmp = args.backbone.strip().split(",") # 没有逗号
            args.backbone = [str(Seq(el).reverse_complement()) for el in tmp] # 最终取值['AGGCTAGTCCGT']
        else:
            args.backbone = []

    # Pad each exon equal to guidesize unless
    if args.padSize != -1: # 默认取值-1，所以进入下选择支
        ...
        if args.MODE == TALENS:
            ...
            padSize = args.guideSize # 把padSize设置为20

    # Set default functions for different modes  
    if args.MODE == CRISPR:
        # Set mismatch checking policy
        (allowedMM, countMM) = getMismatchVectors(args.PAM, args.guideSize, args.uniqueMethod_Cong) # 取NGG,23,False
        allowed = getAllowedFivePrime(args.fivePrimeEnd) # 取NN
        ...
        guideClass = Cas9 if not ISOFORMS else Guide # 因为ISOFORMS是False,所以是Cas9对象
        sortOutput = sort_CRISPR_guides # 只是一个按分数排序的函数而已: def sort_CRISPR_guides(guides):return sorted(guides, key=attrgetter('score'))
    elif args.MODE == CPF1: # 默认取值CRISPR,跳过这个选择块
    ...
    elif args.MODE == TALENS: # 默认取值CRISPR,跳过这个选择块
    ...
    elif args.MODE == NICKASE:  # 默认取值CRISPR,跳过这个选择块
    ...
    if args.database: # 默认取None,所以不进入选择块，进入下选择支
        cdb = connect_db(args.database)
        db = cdb.cursor()
        use_db = True
    else: # 但是其实也没有什么作用就是了，干脆跳过吧
        db = "%s/%s.gene_table" % (
    ...
    if not os.path.isdir(args.outputDir): # 如果输出文件夹不存在就创造它
        os.mkdir(args.outputDir)

    candidate_fasta_file = '%s/sequence.fa' % args.outputDir
    gene, isoform, gene_isoforms = (None, None, set())
    if args.fasta: # 默认取False
    ...
    if args.rm1perfOff and args.fasta: # rm1perfOff默认取False,跳过这一节
        for guide in results:
            if guide.offTargetsMM[0] > 0:
                guide.score -= SINGLE_OFFTARGET_SCORE[0]

    if ISOFORMS: # 因为ISOFORMS取False,所以跳过这个选择块
        for guide in results:
    ...
    if (args.scoringMethod == "CHARI_2015" or args.scoringMethod == "ALL") and (args.PAM == "NGG" or args.PAM == "NNAGAAW") and (args.genome == "hg19" or args.genome == "mm10") and not ISOFORMS: # 因为打分方式是DOENCH_2014,所以跳过这个选择块
        try:
            #make file to score
    ...
    if (args.scoringMethod == "ZHANG_2019" or args.scoringMethod == "ALL") and (args.PAM == "NGG") and not ISOFORMS: # 因为打分方式是DOENCH_2014,所以跳过这个选择块
        try:
    ...
    if (args.scoringMethod == "KIM_2018" or args.scoringMethod == "ALL") and args.PAM in "TTTN" and not ISOFORMS and args.MODE == CPF1: # 因为打分方式是DOENCH_2014,所以跳过这个选择块
    ...
    if (args.scoringMethod == "DOENCH_2016" or args.scoringMethod == "ALL") and not ISOFORMS and args.MODE == CRISPR: # 因为打分方式是DOENCH_2014,所以跳过这个选择块
        # noinspection PyBroadException
        try:
            with warnings.catch_warnings(record=True):
                warnings.simplefilter("ignore")
    ...
    if args.repairPredictions is not None and not ISOFORMS and args.MODE == CRISPR: # repairPredictions取默认值None,所以跳过这个选择块
    ...
    if args.MODE == CRISPR or args.MODE == CPF1 or ISOFORMS:
        cluster = 0 # 设置cluster为0
    elif args.MODE == TALENS: # 因为模式是CRISPR,取第一个选择支
    ...
    elif args.MODE == NICKASE: # 因为模式是CRISPR,取第一个选择支
    ...
    if args.makePrimers: # 这个也是取默认值False,所以这一节其实也可以跳过
        if args.fasta: # 取默认值False
    ...
    if ISOFORMS: # 因为ISOFORMS取False,进入下选择支
        ...
        if args.MODE == CRISPR:
            ...
            if args.scoringMethod == "ALL": # 因为打分方式是DOENCH_2014,选下面那个选择支
    ...
    # FASTA file
    geneFile = open('%s/gene_file.fa' % args.outputDir, 'w') # 因为最终结果只留那个tsv文件，所以跳过这一块
    geneFile.write(">%s\n" % args.targets)
    geneFile.write(fastaSequence)
    geneFile.close()

    # Visualize with json
    if args.jsonVisualize: # 只和可视化有关，无视
    ...
    for fl in os.listdir(args.outputDir): # 只是用来删去.sam文件,不过其实最后也会手动清
        if fl.endswith(".sam"):
