...
parser.add_argument(
        "--region",required=True,help="The target region input use bed format."
    )
parser.add_argument(
...
try:
    region = pd.read_csv(parser.parse_args().region,sep='\t',header=None)
    region.columns = ['chr','start','end']
    regions = []
    if True:
        ...
        for i,row in region.iterrows():
            chromo,start,end = row['chr'],row['start'],row['end']
            regions.append(f'{chromo}:{start}-{end}')
            if not chromo in [f'chr{i}' for i in range(1,23)]+['chrX','chrY']:
                continue
            if start is None:
                continue
            if end is None:
